    ...
    <div className="flex flex-col md:flex-row min-h-screen bg-slate-100 text-slate-900 overflow-x-hidden relative pb-24">
      ...
      {pathname === '/admin/homepage' && (
        dockCollapsed ? (
          ...
          <button
            onClick={toggleDock}
            className="fixed bottom-3 left-4 z-50 bg-white/95 text-slate-900 backdrop-blur-xl border-2 border-[#00A0FF] shadow-[0_10px_30px_rgba(0,160,255,0.25)] rounded-full px-4 py-2 flex items-center gap-2.5 text-xs font-heading font-black hover:scale-105 transition-all cursor-pointer animate-in fade-in slide-in-from-left duration-200"
            title="Dérouler la bibliothèque de modèles de blocs"
          >
            <span className="w-6 h-6 rounded-lg bg-[#00A0FF] text-white flex items-center justify-center text-xs font-black shadow-xs">
              📦
            </span>
            <span className="text-[#00A0FF] tracking-wider uppercase font-black">MODÈLES DE BLOCS (10)</span>
            <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-[#00A0FF]/15 text-[#00A0FF] border border-[#00A0FF]/30 font-mono font-bold">
              + Dérouler
            </span>
          ...
          <div className="fixed bottom-2 left-1/2 -translate-x-1/2 z-50 w-[98%] max-w-[1440px] bg-white/95 text-slate-900 backdrop-blur-xl border-2 border-[#00A0FF] shadow-[0_20px_50px_rgba(15,23,42,0.2)] rounded-3xl p-2 sm:px-3 flex items-center justify-between gap-1.5 animate-in slide-in-from-bottom duration-300">
            {/* LEFT COMPACT TITLE BADGE */}
            <div className="flex items-center gap-1.5 shrink-0 pr-2 border-r border-slate-200">
              <span className="w-7 h-7 rounded-lg bg-[#00A0FF] text-white flex items-center justify-center font-black text-xs shadow-md">
                📦
              </span>
              <div className="flex items-center gap-1">
                ...
                <span className="text-[9px] font-mono font-bold bg-[#00A0FF]/15 text-[#00A0FF] px-1.5 py-0.5 rounded-full border border-[#00A0FF]/30">
                  10
                </span>
              </div>
            </div>
            ...
            >
              {BLOCK_MODELS_DRAGGABLE.map((b) => (
                <button
                  ...
                  className={`px-2.5 py-1.5 rounded-xl text-[10px] font-heading border cursor-grab active:cursor-grabbing flex items-center gap-1 shrink-0 transition-all shadow-xs hover:scale-105 active:scale-95 ${b.color}`}
