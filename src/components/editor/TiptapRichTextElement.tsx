...
export function TiptapRichTextElement({
  ...
  return (
    ...
    >
      {/* TIPTAP PROSEMIRROR NATIVE BUBBLE MENU (NOTION/MEDIUM FLOATING TOOLBAR) */}
      <BubbleMenu
        editor={editor}
        tippyOptions={{ duration: 150, placement: 'top', zIndex: 999999 }}
        className="bg-white text-slate-900 rounded-full shadow-2xl p-1.5 flex items-center gap-1 border-2 border-slate-200 select-none"
      >
        {/* B GRAS */}
        <button
          type="button"
          ...
          className={`p-1.5 hover:bg-slate-100 rounded-full transition-colors ${
            editor.isActive('bold') ? 'bg-slate-200 text-purple-700' : 'text-slate-800'
          }`}
          title="Gras"
        >
        ...
        <button
          ...
          className={`p-1.5 hover:bg-slate-100 rounded-full transition-colors ${
            editor.isActive('italic') ? 'bg-slate-200 text-purple-700' : 'text-slate-800'
          }`}
          title="Italique"
        >
